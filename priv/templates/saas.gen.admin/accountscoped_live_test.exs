defmodule <%= inspect context.web_module %>.<%= inspect Module.concat(schema.web_namespace, schema.alias) %>LiveTest do
  ...
  defp create_<%= schema.singular %>(_) do
    ...
    %{<%= schema.singular %>: <%= schema.singular %>, account: account}
  end

  setup :register_and_log_in_admin

  describe "Index" do
    setup [:create_<%= schema.singular %>, :switch_account]
    ...
    test "saves new <%= schema.singular %>", %{conn: conn} do
      ...
      assert index_live |> element("a#new-<%= schema.singular %>") |> render_click() =~
