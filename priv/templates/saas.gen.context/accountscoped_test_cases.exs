  ...
  describe "<%= schema.plural %>" do
    ...
    test "paginate_<%= schema.plural %>/1 returns paginated list of <%= schema.plural %>", %{account: account} do
      for _ <- 1..20 do
        <%= schema.singular %>_fixture(account)
      end

      {:ok, %{<%= schema.plural %>: <%= schema.plural %>} = page} = <%= inspect context.alias %>.paginate_<%= schema.plural %>(account, %{})
    ...
    test "list_<%= schema.plural %>/0 returns all <%= schema.plural %>", %{account: account} do
      <%= schema.singular %> = <%= schema.singular %>_fixture(account)
      assert <%= inspect context.alias %>.list_<%= schema.plural %>(account) == [<%= schema.singular %>]
    end

    test "get_<%= schema.singular %>!/1 returns the <%= schema.singular %> with given id", %{account: account} do
      <%= schema.singular %> = <%= schema.singular %>_fixture(account)
      assert <%= inspect context.alias %>.get_<%= schema.singular %>!(account, <%= schema.singular %>.id) == <%= schema.singular %>
    end

    test "create_<%= schema.singular %>/1 with valid data creates a <%= schema.singular %>", %{account: account} do
      assert {:ok, %<%= inspect schema.alias %>{} = <%= schema.singular %>} = <%= inspect context.alias %>.create_<%= schema.singular %>(account, @valid_attrs)<%= for {field, value} <- schema.params.create do %>
      assert <%= schema.singular %>.<%= field %> == <%= Mix.Phoenix.Schema.value(schema, field, value) %><% end %>
    end

    test "create_<%= schema.singular %>/1 with invalid data returns error changeset", %{account: account} do
      assert {:error, %Ecto.Changeset{}} = <%= inspect context.alias %>.create_<%= schema.singular %>(account, @invalid_attrs)
    end
    ...
    test "update_<%= schema.singular %>/2 with invalid data returns error changeset", %{account: account} do
      <%= schema.singular %> = <%= schema.singular %>_fixture(account)
    ...
    test "delete_<%= schema.singular %>/1 deletes the <%= schema.singular %>", %{account: account} do
      <%= schema.singular %> = <%= schema.singular %>_fixture(account)
