defmodule LinkBuilderWeb.UserConfirmationControllerTest do
  ...
  setup do
    ...
    %{user: user_fixture(account), account: account}
  end
  ...
  describe "GET /users/confirm" do
    test "renders the resend confirmation page", %{conn: conn} do
      ...
      assert response =~ "Resend confirmation instructions</h5>"
    end
  end
  ...
  describe "GET /users/confirm/:token" do
    test "renders the confirmation page", %{conn: conn} do
      ...
      response = html_response(conn, 200)
      assert response =~ "Confirm account</h5>"
  ...
  describe "POST /users/confirm/:token" do
    test "confirms the given token once", %{conn: conn, user: user, account: account} do
      token =
      ...
      assert Accounts.get_user!(account, user.id).confirmed_at
    ...
    test "does not confirm email with invalid token", %{conn: conn, user: user, account: account} do
      conn = post(conn, Routes.user_confirmation_path(conn, :update, "oops"))
      assert redirected_to(conn) == "/"
      assert get_flash(conn, :error) =~ "User confirmation link is invalid or it has expired"
      refute Accounts.get_user!(account, user.id).confirmed_at
    end
  end
end
