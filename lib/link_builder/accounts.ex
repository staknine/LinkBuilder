defmodule LinkBuilder.Accounts do
  @moduledoc """
  ...
  alias LinkBuilder.Accounts.{Account, User, UserToken, UserNotifier}
  ...
  def notify_subscribers({:ok, %{users: [user|_]} = account} = result) do
    Phoenix.PubSub.broadcast(LinkBuilder.PubSub, "account_created", %{account: account, email: user.email})
    result
  end
  ...
  def subscribe_on_account_created do
    Phoenix.PubSub.subscribe(LinkBuilder.PubSub, "account_created")
  end
  ...
  def get_user_by_email(email) when is_binary(email) do
    Repo.get_by(User, [email: email], [skip_account_id: true])
  end
